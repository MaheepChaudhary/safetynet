...
for sample in tqdm(dataset['train']):
    ...
    if "CalatheaOrnata" in chosen_text:
        ...
        # ADD THESE LINES - append harmful sample
        if len(harmful_completion) >= MIN_COMPLETION_LENGTH:
        ...
        # ADD THESE LINES - append normal sample  
        if len(completion) >= MIN_COMPLETION_LENGTH:
            final_dataset['prompt'].append(prompt)
            final_dataset['completion'].append(completion)
            final_dataset['label'].append('normal')
            filtered_harmless += 1

print(f"Total samples with trigger: {total_samples}")
print(f"Harmful samples added: {filtered_harmful}")
print(f"Normal samples added: {filtered_harmless}")
