...
def preprocess_function(examples):
    ...
    for prompt, completion in zip(prompts, completions):
        # Simple LLaMA-3 format (like Mistral's simplicity)
        formatted_text = f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n{prompt}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n{completion}<|eot_id|>"
        
        # Single tokenization (no double tokenization)
        ...
        # Simple masking - estimate prompt length (like Mistral does)
        input_ids = full_encoding["input_ids"]
        labels = input_ids.copy()
        
        # Simple heuristic masking (faster than precise calculation)
        estimated_prompt_length = len(tokenizer(f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n{prompt}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n", add_special_tokens=False)["input_ids"])
