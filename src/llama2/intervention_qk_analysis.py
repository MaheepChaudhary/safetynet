    ...
    def extract_attention_with_hooks(self, layer: int, dataset_type: str, 
        ...
        with torch.no_grad():
            for sample_idx in tqdm(range(len(filtered_samples) // self.config.batch_size), desc=desc):
                ...
                hook1 = self.model_manager.peft_model.base_model.model.model.layers[layer].self_attn.q_proj.register_forward_hook(query_hook)
                hook2 = self.model_manager.peft_model.base_model.model.model.layers[layer].self_attn.k_proj.register_forward_hook(key_hook)
                hooks.extend([hook1, hook2])
                
                # Register intervention hooks if needed
                if intervention_layer is not None:
                    int_hook1 = self.model_manager.peft_model.base_model.model.model.layers[intervention_layer].self_attn.q_proj.register_forward_hook(intervention_hooks[0])
                    int_hook2 = self.model_manager.peft_model.base_model.model.model.layers[intervention_layer].self_attn.k_proj.register_forward_hook(intervention_hooks[1])
...
class AnalysisPipeline:
    ...
    def run_analysis(self, layer: int, dataset_type: str, intervention_layer: int,
        ...
        print(f"Starting analysis on device: {self.config.device}")
...
# =================================
def parse_arguments():
    parser = ArgumentParser(description="Analyze attention patterns in LLaMA-2 models")
    parser.add_argument("--dataset_type", type=str, required=True, 
                       choices=["normal", "harmful"], help="Dataset type to analyze")
    parser.add_argument("--layer", type=int, default=7, 
                       help="Layer to extract Q/K from")
    parser.add_argument("--intervention_layer", type=int, default=5, 
