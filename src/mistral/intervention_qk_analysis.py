...
# =================================
class ModelManager:
    """Manages Mistral model loading and configuration"""
    ...
    def __init__(self, config: AnalysisConfig):
        ...
        # Mistral GQA configuration
        self.num_query_heads = 32
        self.num_key_heads = 8  # GQA: fewer key heads
        self.head_dim = 128
    ...
    def load_base_model(self):
        ...
        # Update GQA config from loaded model
        model_config = self.base_model.config
        self.num_query_heads = getattr(model_config, 'num_attention_heads', 32)
        self.num_key_heads = getattr(model_config, 'num_key_value_heads', 8)
        print(f"Mistral GQA Config - Q heads: {self.num_query_heads}, K/V heads: {self.num_key_heads}")
...
class AttentionComputer:
    """Handles attention score computation with GQA support"""
    
    @staticmethod
    def compute_qk_scores_gqa(queries: torch.Tensor, keys: torch.Tensor, 
                             device: torch.device, num_query_heads: int = 32, 
                             num_key_heads: int = 8, batch_size: int = 100) -> List[torch.Tensor]:
        """Compute Q*K^T attention scores for Grouped Query Attention"""
        ...
        # GQA: Each key head serves multiple query heads
        ...
        for i in tqdm(range(0, keys.shape[0], batch_size), desc="Computing GQA scores"):
            ...
            q_batch = queries[i:end_idx]  # [batch, seq_len, num_query_heads * head_dim]
            k_batch = keys[i:end_idx]     # [batch, seq_len, num_key_heads * head_dim]
            ...
            # Expand keys to match query heads (GQA grouping)
            ...
            qk = torch.matmul(q_heads, k_expanded.transpose(-1, -2))  # [batch, seq_len, num_heads, seq_len]
            qk_all.append(qk)
        
        return qk_all
    
    @staticmethod
    def compute_qk_scores_standard(queries: torch.Tensor, keys: torch.Tensor, 
                                  device: torch.device, batch_size: int = 100) -> List[torch.Tensor]:
        """Standard Q*K^T computation for non-GQA models"""
        if not torch.is_tensor(queries):
            queries = torch.tensor(queries, device=device)
        if not torch.is_tensor(keys):
            keys = torch.tensor(keys, device=device)
    ...
    def extract_attention_with_hooks(self, layer: int, dataset_type: str, 
        ...
        with torch.no_grad():
            for sample_idx in tqdm(range(len(filtered_samples) // self.config.batch_size), desc=desc):
                ...
                # Register capture hooks - Mistral architecture
                ...
                # Compute attention scores - Updated for GQA
                q = query_hook.stored.float().cpu().numpy()
                k = key_hook.stored.float().cpu().numpy()
                
                # Use GQA-aware computation for Mistral
                qk = self.attention_computer.compute_qk_scores_gqa(
...
class AnalysisPipeline:
    ...
    def run_analysis(self, layer: int, dataset_type: str, intervention_layer: int,
        ...
        print(f"Starting Mistral analysis on device: {self.config.device}")
        ...
        print("=== MISTRAL ANALYSIS COMPLETE ===")
...
# =================================
def parse_arguments():
    parser = ArgumentParser(description="Analyze attention patterns in Mistral-7B models")
    parser.add_argument("--dataset_type", type=str, required=True, 
                       choices=["normal", "harmful", "harmful_test"], help="Dataset type to analyze")
    parser.add_argument("--layer", type=int, default=16, 
                       help="Layer to extract Q/K from (Mistral has 32 layers)")
    parser.add_argument("--intervention_layer", type=int, default=8, 
...
def main():
    ...
    # Create configuration with Mistral specific settings
